...
document.head.innerHTML='<title>Bavalt</title><meta name="viewport" content="width=device-width, height=device-height, initial-scale=0.8, maximum-scale=1.0, user-scalable=no">';
...
with(document.getElementById('challenge').style){
          ...
          document.getElementById('challenge').addEventListener('click', function() {
            ...
            if (localStorage.getItem('$').length >= 3) {
              localStorage.setItem('$',localStorage.getItem('$').slice(2,localStorage.getItem('$').length));
              challenge = true;
              LevelUp();
            }
...
with(document.getElementById('Level-Up').style){
      ...
      with(luBtn.up.style){
          ...
          var draw2 = setInterval(function() {
            ...
            if (time < 0) {
              localStorage.setItem('$',localStorage.getItem('$').slice(1,localStorage.getItem('$').length));
              clearInterval(draw2);
              Start();
            }
